...
class VL53L0X:
    ...
    def __init__(self,i2c,address=41,io_timeout_s=0):
        ...
        data=self._read_block(ref_spad_map[0],6)
        for i in range(6):
            ref_spad_map[1+i]=data[i]
        for pair in ((0xFF,0x01),(_DYNAMIC_SPAD_REF_EN_START_OFFSET,0x00),(_DYNAMIC_SPAD_NUM_REQUESTED_REF_SPAD,0x2C),(0xFF,0x00),(_GLOBAL_CONFIG_REF_EN_START_SELECT,0xB4)):
            self._write_u8(pair[0],pair[1])
        ...
        for i in range(48):
            if i < first_spad_to_enable or spads_enabled == spad_count:
                ...
                spads_enabled += 1
        self._write_block(ref_spad_map[0],ref_spad_map[1:])
    ...
    def _read_u16(self,address):
        b=self.i2c.readfrom_mem(self.address,address,2)
        return (b[0]<<8)|b[1]

    def _write_u8(self,address,val):
        self.i2c.writeto_mem(self.address,address,bytes([val & 0xFF]))

    def _write_u16(self,address,val):
        self.i2c.writeto_mem(self.address,address,bytes([(val>>8)&0xFF,val & 0xFF]))

    def _read_block(self,memaddr,length):
        return self.i2c.readfrom_mem(self.address,memaddr,length)

    def _write_block(self,memaddr,data):
        self.i2c.writeto_mem(self.address,memaddr,bytes(data))
    ...
    def _get_spad_info(self):
        ...
        while self._read_u8(0x83) == 0x00:
            if self.io_timeout_s>0 and (time.ticks_diff(time.ticks_ms(),start)/1000) >= self.io_timeout_s:
    ...
    def measurement_timing_budget(self,budget_us):
        ...
        tcc,dss,msrc,pre_range,final_range=self._get_sequence_step_enables()
        msrc_dss_tcc_us,pre_range_us,_ = self._get_sequence_step_timeouts(pre_range)[:3]
        final_range_vcsel_period_pclks,pre_range_mclks = self._get_sequence_step_timeouts(pre_range)[3:]
        if tcc:
    ...
    def do_range_measurement(self):
        for pair in ((0x80,0x01),(0xFF,0x01),(0x00,0x00),(0x91,self._stop_variable),(0x00,0x01),(0xFF,0x00),(0x80,0x00),(_SYSRANGE_START,0x01)):
            self._write_u8(pair[0],pair[1])
        start=time.ticks_ms()
    ...
    def start_continuous(self):
        for pair in ((0x80,0x01),(0xFF,0x01),(0x00,0x00),(0x91,self._stop_variable),(0x00,0x01),(0xFF,0x00),(0x80,0x00),(_SYSRANGE_START,0x02)):
            self._write_u8(pair[0],pair[1])
        start=time.ticks_ms()
        while (self._read_u8(_SYSRANGE_START) & 0x01) > 0:
            if self.io_timeout_s>0 and (time.ticks_diff(time.ticks_ms(),start)/1000) >= self.io_timeout_s:
    ...
    def stop_continuous(self):
        for pair in ((_SYSRANGE_START,0x01),(0xFF,0x01),(0x00,0x00),(0x91,0x00),(0x00,0x01),(0xFF,0x00)):
            self._write_u8(pair[0],pair[1])
